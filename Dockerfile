...
CMD cp /var/private/id_rsa /home/${LUSER}/.ssh && chmod 0600 /home/${LUSER}/.ssh/id_rsa && git config --global user.email ${GIT_EMAIL} && git config --global user.name ${GIT_NAME} && if [ ! -d .git ]; then git init && git remote add origin git@github.com:${ORGANIZATION}/${REPOSITORY}.git && git fetch origin master && git checkout master; fi && /usr/bin/bash && git push --follow-tags origin master